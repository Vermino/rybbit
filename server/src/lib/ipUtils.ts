...
export function matchesCIDR(ipAddress: string, cidr: string): boolean {
  try {
    ...
    logger.warn({ error, cidr, ipAddress }, `Error matching CIDR ${cidr} for IP ${ipAddress}`);
    return false;
  }
}
...
export function matchesRange(ipAddress: string, range: string): boolean {
  try {
    ...
    logger.warn({ error, range, ipAddress }, `Error matching range ${range} for IP ${ipAddress}`);
    return false;
  }
}
